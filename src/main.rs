...
fn main() {
    ...
    // A move or re-borrow is allowed after the final use of `print`
    ...
    // An attempt to re-borrow will lead to an error.
    // let re_borrow = &count;
    // ^ TODO: try uncommenting this line.
    inc();

    // The closure no longer needs to borrow `&mut count`. Therefore, it is
    // possible to re-borrow without an error
    let count_re_borrowed = &mut count;
